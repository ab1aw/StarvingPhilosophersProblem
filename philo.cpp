...
// helper function to provide a randomized hungry time for Philos
inline QP::QTimeEvtCtr hungry_time() {
    return static_cast<QP::QTimeEvtCtr>((BSP::random() % BSP::TICKS_PER_SEC)
                                        + (BSP::TICKS_PER_SEC/2U));
}

// helper function to provide a randomized starving time for Philos
inline QP::QTimeEvtCtr starving_time() {
    return static_cast<QP::QTimeEvtCtr>((BSP::random() % BSP::TICKS_PER_SEC)
                                        + (BSP::TICKS_PER_SEC/2U));
}
...
Q_STATE_DEF(Philo, hungry) {
    ...
    switch (e->sig) {
        ...
        //.${AOs::Philo::SM::hungry::TIMEOUT}
        case TIMEOUT_SIG: {
            status_ = tran(&starving);
            break;
        }
        default: {
            status_ = super(&top);
...
//.${AOs::Philo::SM::starving} ...............................................
Q_STATE_DEF(Philo, starving) {
    QP::QState status_;
    switch (e->sig) {
        //.${AOs::Philo::SM::starving}
        case Q_ENTRY_SIG: {
            m_timeEvt.armX(starving_time(), 0U);
            TableEvt *pe = Q_NEW(TableEvt, STARVING_SIG);
            pe->philoNum = PHILO_ID(this);
            AO_Table->POST(pe, this);
            status_ = Q_RET_HANDLED;
            break;
        ...
        //.${AOs::Philo::SM::starving::EAT}
        case EAT_SIG: {
            //.${AOs::Philo::SM::starving::EAT::[Q_EVT_CAST(TableEvt)->philoNum=~}
            if (Q_EVT_CAST(TableEvt)->philoNum == PHILO_ID(this)) {
                status_ = tran(&eating);
            }
            else {
...
//.${AOs::Philo::SM::dead} ...................................................
Q_STATE_DEF(Philo, dead) {
    QP::QState status_;
    switch (e->sig) {
        //.${AOs::Philo::SM::dead}
        case Q_ENTRY_SIG: {
            TableEvt *pe = Q_NEW(TableEvt, DEAD_SIG);
            pe->philoNum = PHILO_ID(this);
            AO_Table->POST(pe, this);
            status_ = Q_RET_HANDLED;
            break;
        }
        //.${AOs::Philo::SM::dead::DONE}
        case DONE_SIG: {
            /* DONE must be for other Philos than this one */
            Q_ASSERT(Q_EVT_CAST(TableEvt)->philoNum != PHILO_ID(this));
            status_ = Q_RET_HANDLED;
